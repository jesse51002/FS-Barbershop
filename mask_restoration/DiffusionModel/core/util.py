...
def tensor2img(tensor, out_type=np.uint8, min_max=(-1, 1), process=True):
    '''
    ...
    if process and out_type == np.uint8:
        img_np = ((img_np+1) * 127.5).round()
        # Important. Unlike matlab, numpy.unit8() WILL NOT round by default.
    
    return img_np.astype(out_type).squeeze()

def postprocess(images, process=True):
    return [tensor2img(image, process=process) for image in images]
